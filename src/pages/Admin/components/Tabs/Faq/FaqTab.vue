  ...
  <div class="tab">
    <div class="tab__content">
      <div class="tab__table">
        <FaqFilters />
        <DataTable
          v-if="adminTableContent.faq.length > 0"
          v-model="model"
          :menuItems="menuItems"
          :headers="tableHeadersFaq"
          :data="adminTableContent.faq"
          :sortOrder="sortOrder"
          :sortValue="sortVal"
          markAsSelected
        />
...
<script setup lang="ts">
  ...
  const { tableHeadersFaq } = useAdminTable();

  const editFaq = () => {};
  const deleteFaq = () => {};
  const { menuItems } = useAdminHeaderMenu(editFaq, deleteFaq);
