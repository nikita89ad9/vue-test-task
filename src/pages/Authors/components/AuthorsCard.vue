    ...
    <div class="author__description author-description">
      <div class="author-description__content">
        <div class="author-description__header">
          <div>
            <h4 class="author-description__name">
              {{ userName }}
            </h4>
            <p class="author-description__position">{{ author.jobTitle ?? 'Employee' }}</p>
          </div>
          <BaseButton
            class="author-description__button"
            @click.prevent="handleClickSubscribe"
          >
            {{ $t('general.subscribe') }}
          </BaseButton>
        </div>
        <p class="author-description__text">
      ...
      <BaseButton
        ...
        @click.stop="handleClickSubscribe"
      >
        {{ $t('general.subscribe') }}
      </BaseButton>
    </div>
...
<script setup lang="ts">
  ...
  const userName = computed(() => {
    return props.author.name ? `${props.author.name} ${props.author.surname}` : props.author.email;
  });

  const avatarUrl = computed(() =>
    props.author.avatar ? getFilePath(props.author.avatar.mediaUrl) : '/avatarSample.png'
  );
  ...
  const redirectToProfile = () => {
    router.push({ name: 'Profile', params: { profileID: props.author.id } });
  };
</script>
